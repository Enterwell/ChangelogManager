...
describe("Merge Changelog Test Suite", function() {
    ...
    describe("invalid inputs", function() {
        ...
        it("should fail if input semantic version is not correct format", (done: Mocha.Done) => {
            ...
            // Act 
            let tp: string = path.join(__dirname, "L0InvalidSemanticVersion.js");
            let tr: ttm.MockTestRunner = new ttm.MockTestRunner(tp);
            ...
            runValidations(() => {
             ...
             assert.strictEqual(tr.warningIssues.length, 0, "should have no warnings");
             assert.strictEqual(tr.errorIssues.length, 1, "should have 1 error issue");
             assert(tr.errorIssues[0].startsWith("Error occurred while running executable"), "executable should have correct message");
            }, 
            tr, done);
        });
    })
